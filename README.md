...
Tile and screen-edge warps are resolved from all 529 original `warpSources.s` and `warpDestinations.s` records, including cross-group overworld/interior travel. Room teleports reproduce the original transition modes: gradual or instant white-out, the 32-frame fade into the destination, Link's 28-frame scripted interior entry, and the 16-frame walk offscreen when leaving. Press **H** to stand below the left house door in present room `47`; walk up to enter room `2:ea`, then walk down through the interior's bottom opening to return to the same exterior door.
