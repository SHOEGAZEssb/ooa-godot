...
public sealed class BreakableTileDatabase
{
        ...
        int GashaMaturity)
    {
            ...
            Func<Vector2I, int?>? linkedRoomNeighbor = null)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Breakable tile ${Tile:x2} uses invalid linked room-flag " +
                    $"action ${RoomFlagAction:x2}.");
            }

            int directionCode = RoomFlagAction & 0x0f;
            (Vector2I direction, byte roomFlag, byte oppositeRoomFlag) =
                directionCode switch
                {
                    ...
                    _ => throw new InvalidOperationException(
                        $"Breakable tile ${Tile:x2} uses invalid linked direction " +
                        $"${directionCode:x2}.")
                };
            if (linkKind == 0x40 && direction.Y != 0)
            {
                throw new InvalidOperationException(
                    $"Indoor linked breakable tile ${Tile:x2} uses unsupported " +
                    $"vertical action ${RoomFlagAction:x2}.");
            }
            int neighbor = linkedRoomNeighbor?.Invoke(direction) ??
                throw new InvalidOperationException(
                    $"Breakable tile ${Tile:x2} in room {group:x1}:{room:x2} " +
                    $"requires a linked-room neighbor for action ${RoomFlagAction:x2}.");
