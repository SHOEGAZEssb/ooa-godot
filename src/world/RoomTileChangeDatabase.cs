...
public sealed class RoomTileChangeDatabase
{
    ...
    private readonly record struct Condition(ConditionKind Kind, int A, int B, int C);
    private readonly record struct Operation(OperationKind Kind, int[] Values);
    private readonly record struct Rule(Condition[] Conditions, Operation[] Operations);
