...
{
    private sealed record Frame(Texture2D Texture, Vector2 Offset, int Duration);
    ...
    private sealed class Animator
    {
        ...
        public Animator(IReadOnlyList<Frame> frames, bool loop = false)
        {
            if (frames.Count == 0)
    ...
    private readonly List<Frame> _expand;
    private readonly List<Frame> _contract;
    private readonly List<Frame> _beamIntro;
    private readonly List<Frame> _beamLoop;
    private readonly List<Frame> _beamContract;
    private readonly List<Frame> _trail;
    private readonly List<Frame> _sparkle;
        ...
        bool usesIndoorBeamPalette)
    {
        ...
        _contract = WithoutTerminal(BuildFrames(
            ...
            data.PrimaryTileBase, data.PrimaryPalette));
        List<Frame> beamIntro = BuildFrames(
        ...
        if (_source && _primaryExpanded && !_primaryContracting && _primary is not null)
        {
            if (!_sourceExpandedThisFrame)
            {
                if (_sourceCounter >= 36 && (_sourceCounter & 0x07) == 0)
                {
                    ...
                    TimeWarpEffectDatabase.ParticleRecord record = _data.Particles[index];
    ...
    private static List<Frame> WithoutTerminal(List<Frame> frames)
    {
