...
public partial class RingMenuScreen : Node2D
{
    ...
    private static void DrawTileToImage(
        Image output, Image source, int sourceTile, byte flags, Color[,] palette,
        int destinationX, int destinationY, bool interleaved, bool spriteEncoding)
    {
        bool flipX = (flags & 0x20) != 0;
        bool flipY = (flags & 0x40) != 0;
        ...
        for (int x = 0; x < 8; x++)
        {
            Color pixel = source.GetPixel(
                tileX + (flipX ? 7 - x : x), tileY + (flipY ? 7 - y : y));
    ...
    private static int PaletteShade(Color sourceColor, bool spriteEncoding) =>
        spriteEncoding
            ...
            : TwoBitShade(sourceColor);

    private static Image LoadPng(string path) => OracleGraphicsCache.LoadImage(path);
    ...
    private static Color[,] LoadPalette(string path, int count)
    {
