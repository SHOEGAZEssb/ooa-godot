...
internal sealed class SpiritsGraveDatabase
{
    ...
    internal readonly record struct ObjectRecord(
        int Group,
        int Room,
        int Order,
        ObjectKind Kind,
    ...
    internal readonly record struct EnemyRecord(
        int Id,
        int SubId,
        string[] Sprites,
        int TileBase,
    ...
    private static Condition ParseCondition(GeneratedTableRow row, int column) =>
        row.RequiredString(column) switch
        {
            "always" => Condition.Always,
            "item-clear" => Condition.ItemClear,
            "flag80-clear" => Condition.Flag80Clear,
