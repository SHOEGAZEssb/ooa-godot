...
internal partial class LynnaShopItem : TransitionOffsetNode2D
{
    ...
    public LynnaShopDatabase.ItemRecord Record { get; private set; }
    ...
    public void Initialize(
        LynnaShopDatabase.StockRecord stock,
        OracleRoomData room)
    {
        Record = stock.Item;
        Order = stock.Order;
        ...
        foreach (OracleGraphicsCache.AnimationFrameDefinition sourceFrame in
            animation.Frames)
        {
            ...
            _frames.Add(new Frame(texture, offset, sourceFrame.Duration));
        }
        if (_frames.Count == 0)
