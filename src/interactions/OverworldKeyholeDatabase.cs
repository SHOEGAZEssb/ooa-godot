...
internal sealed class OverworldKeyholeDatabase
{
    ...
    internal readonly record struct ConstantsRecord(
        byte RoomFlag,
        byte InformativeMask,
        int PushCounter,
        int OpenSound,
        int NoKeyTextId,
        string NoKeyMessage,
    ...
    private readonly Dictionary<(int Group, int Room), Record> _records = new();
    ...
    internal bool TryGet(int group, int room, out Record record) =>
    ...
    private void Validate()
    {
        if (_records.Count != 6 || _tiles.Count != 3 ||
            !_records.TryGetValue((0, 0x5c), out Record graveyard) ||
            graveyard is not
            {
