...
internal sealed class DarkRoomDatabase
{
    ...
    public DarkRoomDatabase()
    {
        ...
        foreach (GeneratedTableRow row in interactions.Rows)
        {
            ...
            {
                "handler" => ObjectKind.Handler,
                "reward" => ObjectKind.Reward,
        ...
        IReadOnlyList<Record> roomA8 = GetRoomRecords(5, 0xa8);
        IReadOnlyList<Record> roomEd = GetRoomRecords(5, 0xed);
        if (RecordCount != 3 || _constants.Count != 19 ||
            roomA8.Count != 1 || roomA8[0] != new Record(
                5, 0xa8, 0, ObjectKind.Handler, 0x08, 0x00,
                -1, -1, 0x00, 0, "-", "darkRoomHandler.s:partCode08") ||
            roomEd.Count != 2 || roomEd[0] != new Record(
                5, 0xed, 0, ObjectKind.Reward, 0xdc, 0x00,
                0x48, 0x78, 0x00, 2, "TREASURE_OBJECT_GRAVEYARD_KEY_00",
                "miscellaneous2.s:interactiondc_subid00") ||
            roomEd[1] != new Record(
                5, 0xed, 1, ObjectKind.Handler, 0x08, 0x00,
