...
internal sealed class GashaSpotDatabase
{
    ...
    internal VisualRecord NutVisual { get; }
        ...
        long animationTick)
    {
        ...
        var dynamicTiles = new Dictionary<int, OracleRoomData.DynamicBackgroundTile>();
        ...
        long animationTick)
    {
        ...
        var tiles = new Dictionary<int, OracleRoomData.DynamicBackgroundTile>();
        ...
        for (int index = 0; index < record.SourceCount; index++)
        {
            tiles[0xa0 + record.DestinationStart + index] =
                new OracleRoomData.DynamicBackgroundTile(
    ...
    private void LoadRewards()
    {
        ...
        foreach (GeneratedTableRow row in table.Rows)
        {
            ...
            _rewards[type] = new RewardRecord(
                ...
                new VisualRecord(row.RequiredString(4), row.UnsignedDecimal(5),
                    row.UnsignedDecimal(6), row.RequiredString(7)));
        }
    }

    private static VisualRecord LoadNutVisual()
    {
    ...
    internal readonly record struct SpotRecord(
        int Group, int Room, int SubId, int Y, int X, int Rank,
        string Ground, int ReplacementTile)
    {
    ...
    internal readonly record struct RewardRecord(
        int Type, int TreasureId, int Parameter, int TextId, VisualRecord Visual);

    private readonly record struct DisappearanceRecord(
        int Phase, int SourceStart, int SourceCount, int DestinationStart, byte[] TileMap);
}
