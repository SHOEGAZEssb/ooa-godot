...
/// </summary>
internal static class WingDungeonMinecartState
{
    ...
    internal static void EnsureInitialized(
        ...
        IReadOnlyList<WingDungeonMinecartRecord> records)
    {
        if (IsActive(state))
            return;
        ...
        IReadOnlyList<WingDungeonMinecartRecord> records)
    {
        ...
        ClearActive(state);
        foreach (WingDungeonMinecartRecord record in records)
            WriteStatic(state, record.Slot, record.Room, record.Y, record.X);
    }

    internal static IEnumerable<WingDungeonActiveMinecart> StationaryInRoom(
        OracleRuntimeState state,
        int room)
    {
        for (int slot = 0; slot < StaticSlotCount; slot++)
        {
            ...
            yield return new WingDungeonActiveMinecart(
                slot,
                room,
    ...
    internal static bool TryGetRide(
        OracleRuntimeState state,
        int room,
        out WingDungeonActiveMinecart cart)
    {
        if (IsActive(state) &&
            state.ReadWramByte(ActiveRoom) == room)
        {
            cart = new WingDungeonActiveMinecart(
                Slot: -1,
                room,
        ...
        Vector2 position)
    {
        ...
        if (slot < 0)
            throw new InvalidOperationException(
                "Wing Dungeon has no free wStaticObjects slot for its minecart.");
        WriteStatic(
            state,
            slot,
            room,
...
internal readonly record struct WingDungeonActiveMinecart(
    int Slot,
    int Room,
    int Y,
    int X,
