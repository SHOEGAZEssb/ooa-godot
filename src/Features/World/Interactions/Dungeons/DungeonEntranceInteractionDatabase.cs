...
internal sealed class DungeonEntranceInteractionDatabase
{
    ...
    internal IReadOnlyList<DungeonEntranceInteractionDatabaseVisualRecord> EyeVisuals => _eyeVisuals;
    internal DungeonEntranceInteractionDatabaseVisualRecord PortalVisual => _portalVisual;
    ...
    public DungeonEntranceInteractionDatabase()
    {
        ...
        {
            DungeonEntranceInteractionDatabaseObjectKind kind = row.RequiredString(3) switch
            {
                "entry" => DungeonEntranceInteractionDatabaseObjectKind.Entry,
                "eye-spawner" => DungeonEntranceInteractionDatabaseObjectKind.EyeSpawner,
                "miniboss-portal" => DungeonEntranceInteractionDatabaseObjectKind.MinibossPortal,
                _ => throw row.Invalid(
            ...
            if (record.Kind == DungeonEntranceInteractionDatabaseObjectKind.Entry &&
                    (record.Id != 0x12 || record.SubId != 0x00 ||
                     record.Y < 0 || record.X < 0) ||
                record.Kind == DungeonEntranceInteractionDatabaseObjectKind.EyeSpawner &&
                    (record.Id != 0xe2 || record.SubId != 0x01 ||
                     record.Y >= 0 || record.X >= 0) ||
                record.Kind == DungeonEntranceInteractionDatabaseObjectKind.MinibossPortal &&
        ...
        {
            DungeonEntranceInteractionDatabaseVisualRecord visual = new DungeonEntranceInteractionDatabaseVisualRecord(
        ...
        if (entryCount != 16 || PlacementCount != 42 || eyeVisualCount != 8 ||
            ...
            room24[0] is not { Order: 0, Kind: DungeonEntranceInteractionDatabaseObjectKind.Entry, Dungeon: 1 } ||
            room24[1] is not { Order: 1, Kind: DungeonEntranceInteractionDatabaseObjectKind.EyeSpawner } ||
            room24[2] is not { Order: 2, Kind: DungeonEntranceInteractionDatabaseObjectKind.MinibossPortal } ||
