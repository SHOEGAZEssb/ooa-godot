...
/// </summary>
internal sealed partial class WingDungeonBossProjectile
    : TransitionOffsetNode2D, IHostileProjectile
{
    private const int LinkRadius = 6;
    private readonly WingDungeonBossProjectileKind _kind;
    ...
    internal WingDungeonBossProjectile(
        ...
        OracleRoomData room)
    {
        ...
        _speed = spawn.Speed;
        Name = _kind == WingDungeonBossProjectileKind.Fireball
            ? "HeadThwompFireball"
        ...
        if (_kind == WingDungeonBossProjectileKind.Fireball)
        {
            _counter = 30;
            _speedZ = -0x3e0;
        }
    ...
    public Rect2 CollisionBounds
    {
        get
        {
            ...
            Vector2 radii = _kind == WingDungeonBossProjectileKind.Fireball
                ? new Vector2(5, 5)
                : new Vector2(4, 4);
    ...
    public void UpdateFrame(Player player)
    {
        ...
        if (CollisionBounds.Intersects(linkBounds))
        {
            player.ApplyEnemyContactDamage(
                DrawPosition,
                2,
                _kind == WingDungeonBossProjectileKind.Circular
    ...
    public override void _Draw()
    {
        if (Finished)
            return;
        Vector2 offset = _kind == WingDungeonBossProjectileKind.Fireball
...
internal enum WingDungeonBossProjectileKind
{
    Fireball,
    Circular
}
