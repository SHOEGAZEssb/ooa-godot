...
public partial class EnemyDeathPuffEffect : TransitionOffsetNode2D
{
    ...
    private static EnemyDeathPuffEffectDefinition? _definition;

    private List<EnemyDeathPuffEffectFrameRecord> _animation = null!;
        ...
        int enemyId = -1)
    {
        ...
        _paletteIndex = 0;
        DurationFrames = 0;
        foreach (EnemyDeathPuffEffectFrameRecord frame in _animation)
    ...
    private static EnemyDeathPuffEffectDefinition LoadDefinition()
    {
        ...
        List<EnemyDeathPuffEffectFrameRecord> normal = BuildAnimation(
            source, row.RequiredString(3), tileBase, palettes);
        List<EnemyDeathPuffEffectFrameRecord> highKnockback = BuildAnimation(
        ...
        return new EnemyDeathPuffEffectDefinition(palettes, normal, highKnockback);
    }

    private static List<EnemyDeathPuffEffectFrameRecord> BuildAnimation(
        Image source,
        string encodedAnimation,
        int tileBase,
        int[] palettes)
    {
        var animation = new List<EnemyDeathPuffEffectFrameRecord>();
            ...
            OracleGraphicsCache.GetAnimationDefinition(encodedAnimation).Frames)
        {
            ...
            animation.Add(new EnemyDeathPuffEffectFrameRecord(textures, frame.Duration));
        }
        return animation;
    }
}
