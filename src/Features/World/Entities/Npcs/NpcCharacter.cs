...
public partial class NpcCharacter : TransitionOffsetNode2D
{
    ...
    public Vector2I FacingVector => _facing switch
    {
        ...
        NpcCharacterFacing.Right => Vector2I.Right,
        NpcCharacterFacing.Left => Vector2I.Left,
        _ => Vector2I.Down
    };
    ...
    public void UpdateNpc(double delta, Vector2 linkPosition)
    {
        ...
        NpcCharacterFacing desired = Mathf.Abs(difference.X) + Mathf.Abs(difference.Y) < 0x28
    ...
    internal void SetFacingDirection(Vector2I direction)
    {
        ...
        SetFacing(direction == Vector2I.Up ? NpcCharacterFacing.Up
            ...
            : direction == Vector2I.Left ? NpcCharacterFacing.Left
            : NpcCharacterFacing.Down);
        QueueRedraw();
    }
    ...
    private List<NpcCharacterAnimationFrame> CurrentAnimation => _scriptAnimationActive
        ? _scriptAnimation
    ...
    private NpcCharacterFacing GetFacingToward(Vector2 target)
    {
        Vector2 delta = target - Position;
        if (Mathf.Abs(delta.X) > Mathf.Abs(delta.Y))
            return delta.X > 0 ? NpcCharacterFacing.Right : NpcCharacterFacing.Left;
        return delta.Y > 0 ? NpcCharacterFacing.Down : NpcCharacterFacing.Up;
    }

    private void SetFacing(NpcCharacterFacing facing)
    {
        if (_facing == facing)
            return;
        _facing = facing;
    ...
    private int GetFrameColumn()
    {
        ...
        {
            NpcCharacterFacing.Up => 1,
            NpcCharacterFacing.Right => 2,
            NpcCharacterFacing.Left => 2,
            _ => 0
        };
    }
    ...
    {
        foreach (List<NpcCharacterAnimationFrame> animation in _facingAnimations)
            animation.Clear();
        ...
        _facingAnimationLoopStarts[(int)NpcCharacterFacing.Up] = AnimationLoopStart(up);
        _facingAnimationLoopStarts[(int)NpcCharacterFacing.Right] = AnimationLoopStart(right);
        _facingAnimationLoopStarts[(int)NpcCharacterFacing.Down] = AnimationLoopStart(down);
    ...
    private static IEnumerable<NpcCharacterAnimationFrame> BuildAnimation(
        Image source,
    ...
    private static IEnumerable<NpcCharacterAnimationFrame> BuildPositionedAnimation(
        Image source,
        string encodedAnimation,
        int tileBase,
        int basePalette,
    ...
    internal static (Texture2D Texture, Vector2 Offset)
        BuildPositionedOamTextureWithPaletteOverrides(
            Image source,
            string encodedOam,
            int tileBase,
            int basePalette,
        ...
        OamFrame frame = OracleGraphicsCache.GetOrCreateOamFrame(
            ...
            () =>
            {
                (Texture2D texture, Vector2 offset) =
                    BuildPositionedOamTextureUncached(
                        source, encodedOam, tileBase, basePalette,
                        paletteOverride: null, paletteOverrides,
                        sourceGrayscaleInverted, sourceOffset);
                return new OamFrame(texture, offset);
        ...
        int sourceOffset)
    {
        ...
        if (minX == int.MaxValue)
            return (BuildOamTextureUncached(
                ...
                paletteOverrides, sourceGrayscaleInverted, sourceOffset),
                new Vector2(-16, -16));
        ...
        for (int blockIndex = blocks.Length - 1; blockIndex >= 0; blockIndex--)
        {
            ...
            int palette = basePalette ^ (flags & 0x07);
            Color[]? blockPalette = paletteOverrides is not null &&
                paletteOverrides.TryGetValue(
                    palette, out Color[]? overridePalette)
                    ? overridePalette
                    : paletteOverride;
