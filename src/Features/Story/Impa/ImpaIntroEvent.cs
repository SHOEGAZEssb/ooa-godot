...
internal sealed class ImpaIntroEvent : IRoomEvent, ICutsceneCommandHost
{
    ...
    private readonly ImpaIntroEventRecord _record;
    private readonly ImpaHelpEventRecord _helpRecord;
    private readonly ImpaStoneEventRecord _stoneRecord;
    ...
    public bool HasState => _stage != ImpaIntroEventStage.None || _encounterRunner.Active ||
    ...
    public bool BlocksGameplay =>
        _encounterRunner.Active || _stage is not (ImpaIntroEventStage.None or ImpaIntroEventStage.Following) ||
    ...
    public void StartEncounter(OracleRoomData room)
    {
        ...
        _fakeOctoroks.Clear();
        foreach (FakeOctorokRecord record in _database.Octoroks)
        {
    ...
    {
        if (_stage != ImpaIntroEventStage.None || _encounterRunner.Active)
    ...
    public void TransferFollowingActor(int group, OracleRoomData room)
    {
        ...
        NpcRecord record = Actor!.Record with
        {
            Group = group,
            Room = room.Id,
    ...
    private void UpdateEncounterFrame()
    {
        ...
        switch (_stage)
        {
            ...
            case ImpaIntroEventStage.LinkHorizontal:
                ...
                {
                    BeginWait(ImpaIntroEventStage.LinkCenterWait, _record.CenterWaitFrames);
                    break;
                }
                ...
                break;
            case ImpaIntroEventStage.LinkCenterWait:
                if (CountDown())
                {
                    _counter = _record.ApproachFrames;
                    _context.Player.Face(Vector2I.Up);
                    _stage = ImpaIntroEventStage.LinkApproach;
                }
                break;
            case ImpaIntroEventStage.LinkApproach:
                EnsureObjectSpeed(_record.LinkSpeed, 0x28, "Link's room 0:6a approach");
    ...
    void ICutsceneCommandHost.ShowText(int textId, string message)
    {
        ...
        if (_stonePrePushRunner.Active)
        {
            ImpaStoneText stoneText = textId switch
            {
        ...
        if (_stonePostPushRunner.Active)
        {
            ImpaStoneText thanks = _stoneRecord.Texts.Thanks;
    ...
    private void BeginWait(ImpaIntroEventStage stage, int frames)
    {
        _stage = stage;
        _counter = frames;
    }
