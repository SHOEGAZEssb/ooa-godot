...
internal sealed class NayruActorRegistry : IReadOnlyDictionary<string, NpcCharacter>
{
    ...
    public NpcCharacter Spawn(
        ...
        bool solid = false)
    {
        ActorRecord actor = _database.Actor(template);
    ...
    {
        ActorRecord actor = _database.Actor(name);
        TextRecord text = _database.Text(textId);
        NpcRecord record = actor.ToNpcRecord(group, room) with
        {
            TextId = textId,
