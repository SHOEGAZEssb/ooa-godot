...
internal sealed class PreBlackTowerEvent : IRoomEntryEvent, ICutsceneCommandHost
{
    ...
    {
        NpcRecord record = Actor(actor).Record;
    ...
    {
        NpcRecord linked = _actors.TryGetValue("Nayru", out NpcCharacter? placed)
            ? placed.Record
            : FindPlacedNayruRecord();
        NpcRecord record = linked with
        {
    ...
    private NpcRecord FindPlacedNayruRecord()
    {
