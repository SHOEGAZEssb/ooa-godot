...
public sealed partial class ValidationRoot
{
    private void ValidateSpiritsGrave()
    {
        ...
        {
            if (!chests.TryGet(4, room, position, out ChestDatabase.ChestRecord chest) ||
        ...
        if (!breakables.TryGet(
                2, 0x69,
                out BreakableTileDatabase.BreakableTileRecord burnableWall) ||
        ...
        if (chargingRope.State != RopeCharacter.RopeState.Charging ||
        ...
        int chargeUpdates = 0;
        while (chargingRope.State == RopeCharacter.RopeState.Charging &&
               chargeUpdates < 64)
        {
            chargingRope.UpdateFrame(_player.Position);
            chargeUpdates++;
        ...
        if (chargingRope.State != RopeCharacter.RopeState.Wandering ||
        ...
        if (_entities.RandomCalls != ghiniRandomCalls ||
            ghini.State != GhiniCharacter.GhiniState.Choosing)
        {
            throw new InvalidOperationException(
        ...
        if (_entities.RandomCalls != ghiniRandomCalls + 1 ||
            ghini.State != GhiniCharacter.GhiniState.Moving)
        {
            throw new InvalidOperationException(
        ...
        for (int frame = 0;
             frame < 300 && fallingKey.State != GroundTreasurePickup.PickupState.Waiting;
             frame++)
        {
            StepEntities();
        }
        if (fallingKey.State != GroundTreasurePickup.PickupState.Waiting)
        {
            throw new InvalidOperationException(
        ...
        if (!warps.TryGetTileWarp(
                4, 0x1b, 0x2b, 0x45, out WarpDatabase.Warp braceletStairs) ||
        ...
        if (_transitions.IsTransitioning ||
            ...
            !warps.TryGetEdgeWarp(
                ...
                out WarpDatabase.Warp braceletReturn) ||
        ...
        if (_entities.Entities<GroundTreasurePickup>() is not
            [{
                State: GroundTreasurePickup.PickupState.Waiting,
        ...
        pumpkin.ApplySwordHit(
            pumpkin.CollisionBounds, pumpkin.Position, damage: 20);
        if (pumpkin.State != PumpkinHeadBoss.BossState.HeadExposed)
        {
            throw new InvalidOperationException(
        ...
        if (!_playerWorld.TryUseBracelet(_player, primaryButton: false) ||
            !pumpkin.HeadHeld ||
            _bracelet.State != BraceletController.BraceletState.LiftingEntity)
        {
            throw new InvalidOperationException(
        ...
        {
            if (_bracelet.State == BraceletController.BraceletState.Throwing)
            {
        ...
        if (pumpkin.GhostHealth != 6 ||
            pumpkin.State != PumpkinHeadBoss.BossState.HeadExposed ||
            _sound.PlayRequestsFor(OracleSoundEngine.SndBossDamage) != 1)
        {
            throw new InvalidOperationException(
        ...
        for (int frame = 0; frame < 360 &&
             pumpkin.State != PumpkinHeadBoss.BossState.Active;
             frame++)
        {
            StepEntities();
        }
        if (pumpkin.State != PumpkinHeadBoss.BossState.Active ||
        ...
        for (int expectedHealth = 3; expectedHealth >= 0; expectedHealth -= 3)
        {
            ...
            for (int frame = 0; frame < 240 &&
                 ...
                 pumpkin.State != PumpkinHeadBoss.BossState.Dying;
                 frame++)
            {
                if (_bracelet.State == BraceletController.BraceletState.Throwing)
                {
        ...
        if (pumpkin.State != PumpkinHeadBoss.BossState.Dying)
        {
            throw new InvalidOperationException(
        ...
        _sound.ClearPlayRequestAudit();
        StepEntities(181);
        if (wallmaster.State != WallmasterCharacter.WallmasterState.Waiting ||
        ...
        for (int frame = 0; frame < 180 &&
             wallmaster.State != WallmasterCharacter.WallmasterState.Grounded;
             frame++)
        {
            StepEntities();
        }
