...
public sealed partial class ValidationRoot
{
    private void ValidateOverworldKeyholeAndGraveyardGate()
    {
        ...
        for (int frame = 0; frame < 9; frame++)
            Push();
        FailIf(
            _dialogue.IsOpen || _keyholes.RemainingPushFrames != 2 ||
        ...
        for (int frame = 0; frame < 10; frame++)
            Push();
        FailIf(
            _dialogue.IsOpen || !_keyholes.InformativeTextShown,
        ...
        for (int frame = 0; frame < 59; frame++)
            keyEffect.UpdateFrame();
        FailIf(
            keyEffect.Finished || keyEffect.Counter != 1,
            "INTERAC_OVERWORLD_KEY_SPRITE ended before its full apex hold.");
        keyEffect.UpdateFrame();
        FailIf(!keyEffect.Finished, "INTERAC_OVERWORLD_KEY_SPRITE did not delete after its 60-update hold.");
        ...
        for (int frame = 0; frame < 44; frame++)
            StepGate();
        FailIf(
            gate.Counter != 1 || At(0x43) != 0x98 || At(0x45) != 0x9a ||
        ...
        StepGate();
        FailIf(
            _sound.ActiveMusic != roomMusic || gate.CurrentCommandIndex != 7,
        ...
        room = _currentRoom;
        FailIf(
            room.GetMetatile(new Vector2(0x48, 0x38)) != 0x3a ||
