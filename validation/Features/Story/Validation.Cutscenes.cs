...
public sealed partial class ValidationRoot
{
    private void ValidateNewGameIntro()
    {
        ...
        FailIf(
            linkSpin.Length != 8 || linkVanish.Length != 4 || linkArrival.Length != 3 ||
        ...
        intro.Update(1.0 / 60.0);
        FailIf(
            intro.CurrentStage != Stage.PostVanish ||
            intro.StageFrame != 0,
        ...
        for (int frame = 1; frame < record.SummonFrames / 2; frame++)
        {
            FailIf(
                !UpdateNewGameArrival(1.0 / 60.0) || _player.Visible,
                "Link appeared before summon-wave frame 64.");
        }
        FailIf(
            !UpdateNewGameArrival(1.0 / 60.0) || !_player.Visible ||
        ...
        FailIf(_player.NewGameSlowFallFrame != 0, "LINK_ANIM_MODE_FALL frame $e6 ended early.");
        UpdateNewGameArrival(1.0 / 60.0);
        FailIf(_player.NewGameSlowFallFrame != 1, "LINK_ANIM_MODE_FALL did not advance after four updates.");
        for (int frame = 69; frame <= 122; frame++)
            UpdateNewGameArrival(1.0 / 60.0);
        FailIf(
            !_player.IsNewGameSlowFalling || _player.NewGameSlowFallFrame != 2 ||
            _player.NewGameSlowFallZ != -3,
            "Transition $0b diverged before its 59th gravity update.");
        FailIf(
            !UpdateNewGameArrival(1.0 / 60.0) || _player.IsNewGameSlowFalling,
            "Transition $0b did not land on wave frame 123.");
        ...
        FailIf(
            !TryDisplayEraInfoAfterInitialRoomLoad(InitialRoomLoadKind.Ordinary) ||
    ...
    private void ValidateTimePortals()
    {
        ...
        FailIf(
            effectDatabase.TimeWarpSprite != "spr_timeportal" ||
            ...
            effectDatabase.Particles[0] != new ParticleRecord(0x280, -4, 0) ||
            effectDatabase.Particles[7] != new ParticleRecord(0x240, 9, 3) ||
        ...
        IReadOnlyList<PortalRecord> ordinaryRecords =
            database.GetRoomPortals(0, 0x3a);
        FailIf(
            ordinaryRecords.Count != 1 || ordinaryRecords[0].SubId != 0x00 ||
        ...
        FailIf(
            ordinaryPortals.Count != 1 ||
            ordinaryPortals[0].Active ||
            ordinaryPortals[0].Visible ||
            _currentRoom.GetMetatile(new Vector2(0x18, 0x28)) != 0xd7,
            "Room 0:3a's ordinary `$e1:$00 did not remain invisibly " +
            "dormant on its exposed `$d7 marker before Tune of Echoes.");

        IReadOnlyList<PortalRecord> records = database.GetRoomPortals(0, 0x39);
        FailIf(
            records.Count != 1 || records[0].SubId != 0x01 ||
        ...
        FailIf(
            sourceUsesIndoorBeamPalette || !destinationUsesIndoorBeamPalette,
        ...
        FailIf(
            portals.Count != 1 || portals[0].Active ||
        ...
        FailIf(
            !_currentRoom.ReplaceMetatile(portal.Position, 0x3a, 0xd7, (long)_animationTicks),
            "Could not reveal portal-spot metatile `$d7 for validation.");
        _roomView.QueueRedraw();
        _entities.Update(1.0 / 60.0, _player);
        FailIf(!portal.Active, "The `$e1 portal did not initialize after its `$d7 spot was revealed.");
        for (int frame = 0; frame < 6; frame++)
            _entities.Update(1.0 / 60.0, _player);
        FailIf(portal.CurrentFrame != 3, "The portal did not finish its three 2-update intro frames.");
        for (int frame = 0; frame < 6; frame++)
            _entities.Update(1.0 / 60.0, _player);
        FailIf(portal.CurrentFrame != 3, "The portal's three-frame animation loop restarted incorrectly.");
        ...
        _entities.Update(1.0 / 60.0, _player);
        FailIf(
            !IsTransitioning || portal.Visible || _player.Position != portal.Position ||
        ...
        FailIf(
            _transitions.TimeWarpPhaseName != "TimeWarpDissolve" ||
        ...
        FailIf(
            _transitions.TimeWarpPhaseName != "TimeWarpSetup" ||
        ...
        UpdateRoomWarpTransition(1.0 / 60.0);
        FailIf(
            _transitions.TimeWarpPhaseName != "TimeWarpSourceTrail" ||
        ...
        FailIf(
            sourceEffect.SparkleSpawnCount != 1 || sourceEffect.ActiveSparkleCount != 1 ||
        ...
        FailIf(
            sourceEffect.SourceCounter != 1 || sourceEffect.BeamContracting ||
            !sourceEffect.PrimaryVisible || !sourceEffect.BeamVisible,
            "The source ground or purple child began contracting before $dd:$00 counter1 reached zero.");
        UpdateRoomWarpTransition(1.0 / 60.0);
        FailIf(
            sourceEffect.SourceCounter != 0 || !sourceEffect.BeamContracting ||
        ...
        UpdateRoomWarpTransition(1.0 / 60.0);
        FailIf(
            _activeGroup != 1 || _currentRoom.Id != 0x39 ||
        ...
        UpdateRoomWarpTransition(11.0 / 60.0);
        FailIf(
            arrivalEffect.BeamVisible || !arrivalEffect.PrimaryVisible,
    ...
    private void ValidateGraveyardGhostKidsCutscene()
    {
        ...
        FailIf(
            ...
            ghostKids.RedStage != ChildStage.WaitForSignal ||
            ghostKids.GreenStage != ChildStage.InitialWait ||
            ghostKids.BlueStage != ChildStage.WaitForSignal ||
        ...
        FailIf(ghostKids.GreenCounter != 1 || _dialogue.IsOpen, "The green child's wait 30 ended early.");
        StepRoomEventFrames(1);
        const string greenText =
            "No way! It's too\nscary! It's\npitch black\ninside that\ngrave beneath\nthe tree!";
        FailIf(
            !_dialogue.IsOpen || _dialogue.CurrentMessage != greenText ||
            ghostKids.GreenStage != ChildStage.Dialogue,
        ...
        FailIf(ghostKids.GreenCounter != 1 || ghostKids.Signal != 0, "The green post-text wait ended early.");
        StepRoomEventFrames(1);
        FailIf(
            ghostKids.Signal != 1 ||
            ghostKids.GreenStage != ChildStage.WaitForSignal ||
            ghostKids.BlueStage != ChildStage.Jumping ||
        ...
        FailIf(
            !_dialogue.IsOpen || _dialogue.CurrentMessage != blueText ||
            ghostKids.BlueStage != ChildStage.Dialogue,
        ...
        StepRoomEventFrames(1);
        FailIf(
            ghostKids.RedStage != ChildStage.RedFreezeWait ||
        ...
        FailIf(
            !_dialogue.IsOpen || _dialogue.CurrentMessage != "Besides..." ||
            red.CurrentScriptAnimationSource != red.Record.DownAnimation,
        ...
        FailIf(
            ghostKids.RedStage !=
                ChildStage.RedPostFirstWait ||
            ghostKids.RedCounter != record.RedPostTextWait,
            "TX_2512 did not install wait 30 on close.");
        StepRoomEventFrames(record.RedPostTextWait);
        FailIf(
            ghostKids.RedStage != ChildStage.RedTurnLeftWait ||
            ghostKids.RedCounter != record.RedFreezeWait ||
            red.CurrentScriptAnimationSource != red.Record.LeftAnimation,
            "The red child did not turn left and install the second wait 32.");
        StepRoomEventFrames(record.RedFreezeWait);
        FailIf(
            !_dialogue.IsOpen || _dialogue.CurrentMessage != "It might come\nout...",
            "The second frozen wait did not show TX_2513.");

        _dialogue.Close();
        StepRoomEventFrames(1);
        StepRoomEventFrames(record.RedPostTextWait);
        FailIf(
            ghostKids.RedStage != ChildStage.RedTurnUpWait ||
            ghostKids.RedCounter != record.RedFreezeWait ||
            red.CurrentScriptAnimationSource != red.Record.UpAnimation,
            "The red child did not turn up and install the third wait 32.");
        StepRoomEventFrames(record.RedFreezeWait);
        FailIf(
            !_dialogue.IsOpen || _dialogue.CurrentMessage != "...The ghost.",
            "The third frozen wait did not show TX_2514.");

        _dialogue.Close();
        StepRoomEventFrames(1);
        FailIf(
            ghostKids.RedStage != ChildStage.RedFinalWait ||
            ghostKids.RedCounter != record.RedFinalWait,
            "TX_2514 did not install the final wait 60.");
        StepRoomEventFrames(record.RedFinalWait - 1);
        FailIf(
            ghostKids.RedCounter != 1 || ghostKids.Signal != 2 ||
            _entities.RandomCalls != randomCallsBeforeShake,
            "The final wait ended or consumed shared RNG before update 60.");
        StepRoomEventFrames(1);
        FailIf(
            ghostKids.Signal != 3 ||
            ghostKids.RedStage != ChildStage.Shaking ||
            ghostKids.GreenStage != ChildStage.Shaking ||
            ghostKids.BlueStage != ChildStage.Shaking ||
        ...
        FailIf(
            ghostKids.RedCounter != 1 || ghostKids.GreenCounter != 1 ||
        ...
        FailIf(
            ghostKids.RedStage != ChildStage.Fleeing ||
            ghostKids.GreenStage != ChildStage.Fleeing ||
            ghostKids.BlueStage != ChildStage.Fleeing ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            ghostKids.RedStage != ChildStage.FleeEndPending ||
            ghostKids.GreenStage != ChildStage.FleeEndPending ||
            ghostKids.BlueStage != ChildStage.FleeEndPending ||
            red.Position != redFleeStart + Vector2.Right * 110 ||
            green.Position != greenFleeStart + Vector2.Right * 110 ||
            blue.Position != blueFleeStart + Vector2.Right * 110,
        ...
        StepRoomEventFrames(1);
        FailIf(
            ghostKids.HasState || !ghostKids.Completed || _player.CutsceneControlled ||
        ...
        FailIf(
            ghostKids.HasState || reloadedChildren.Count != 3 ||
    ...
    private void ValidateEnterPastEvent()
    {
        ...
        FailIf(
            enterPast.ZFixed != -0xb0 || villager.ScriptDrawOffset.Y != -1,
        ...
        FailIf(
            villager.Position != new Vector2(0x18, 0x2e) ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            villager.Position != new Vector2(0x18, 0x2f) ||
        ...
        FailIf(
            villager.Position != new Vector2(0x18, 0x38) || enterPast.Counter != 1,
        ...
        NpcCharacter? completedVillager = _npcNodes.Find(npc =>
            npc.Record.Id == record.InteractionId && npc.Record.SubId == record.SubId);
        FailIf(
            completedVillager is null || completedVillager.Active || enterPast.HasState ||
            _player.CutsceneControlled,
        ...
        FailIf(
            starts[3].ScriptUpdate != starts[2].ScriptUpdate ||
    ...
    private void ValidateImpaIntroEncounter()
    {
        ...
        FailIf(
            !impaEvent.HelpWaitingAtEdge || _roomEvents.Active,
            "Room 0:7a did not arm INTERAC_MISCELLANEOUS_1 $6b:$00.");
        impaEvent.UpdateHelpFrame(upPressed: true);
        FailIf(_dialogue.IsOpen, "Impa's help text triggered before Link's Y coordinate was below $07.");
        ...
        StepRoomEventFrames(1);
        FailIf(
            !_transitions.ScrollActive || _activeGroup != 0 || _currentRoom.Id != 0x6a,
        ...
        FailIf(
            impa is null || !_roomEvents.Active || impa.Position != new Vector2(0x48, 0x38) ||
            octoroks.Count != 3 ||
        ...
        FailIf(
            _player.Position != new Vector2(0x47, 0x76) ||
            _player.FacingVector != Vector2I.Right,
            "Link did not approach center X=$48 at one pixel per SPEED_100 update.");
        StepRoomEventFrames(1);
        StepRoomEventFrames(1);
        FailIf(
            impaEvent.Counter != 4 || _player.Position != new Vector2(0x48, 0x76),
            "Link did not install the four-update center wait at X=$48.");
        StepRoomEventFrames(3);
        FailIf(
            impaEvent.Counter != 1 || _player.Position.Y != 0x76,
            "Link's four-update center wait ended early.");
        StepRoomEventFrames(1);
        StepRoomEventFrames(45);
        FailIf(
            impaEvent.Counter != 1 || _player.Position != new Vector2(0x48, 0x49),
            "Link's $2e-update upward approach ended early.");
        StepRoomEventFrames(1);
        FailIf(
        ...
        StepRoomEventFrames(1);
        FailIf(
            impaEvent.Counter != 210 || impaEvent.EncounterCommandIndex != 1,
        ...
        FailIf(
            octoroks[0].Position != fakeStarts[0] ||
            octoroks[1].Position != fakeStarts[1] ||
            octoroks[2].Position != fakeStarts[2],
            "A fake Octorok left before its original $14 signal wait.");
        StepRoomEventFrames(1);
        StepRoomEventFrames(59);
        FailIf(
            octoroks[1].Position != fakeStarts[1],
            "Fake Octorok var03=$01 moved during its $3c flee delay.");
        StepRoomEventFrames(1);
        FailIf(
            octoroks[1].Position != fakeStarts[1] ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            octoroks[1].Position != fakeStarts[1] + Vector2.Left * 3,
            "Fake Octorok var03=$01 did not flee left at SPEED_300 after $14+$3c updates.");

        StepRoomEventFrames(129);
        FailIf(_dialogue.IsOpen || impaEvent.Counter != 1, "Impa's 210-update post-signal wait ended early.");
        StepRoomEventFrames(1);
        FailIf(
        ...
        StepRoomEventFrames(29);
        FailIf(
            impaEvent.Counter != 1 || impa!.Position != new Vector2(0x48, 0x38),
            "Impa's 30-update post-text wait ended early.");
        StepRoomEventFrames(1);
        FailIf(
            impaEvent.Counter != 0 || impaEvent.EncounterCommandIndex != 5,
            "Impa's post-text wait did not carry through setspeed on its completion update.");
        StepRoomEventFrames(1);
        FailIf(
            impa.Position != new Vector2(0x48, 0x38) || impaEvent.Counter != 32,
            "Impa moved during the setspeed/movedown script-command updates.");
        StepRoomEventFrames(30);
        FailIf(
            impaEvent.Counter != 2 || impa.Position != new Vector2(0x48, 0x47),
            "Impa did not apply SPEED_080 through the high-coordinate byte for 30 updates.");
        StepRoomEventFrames(1);
        FailIf(
            impaEvent.Counter != 1 || impa.Position != new Vector2(0x48, 0x47),
            "movedown $20 did not retain its final half-pixel fraction.");
        StepRoomEventFrames(1);
        FailIf(
            _saveData.HasRoomFlag(0, 0x6a, OracleSaveData.RoomFlag40),
            "Impa set room flag $40 before counter2 reached zero.");
        StepRoomEventFrames(1);
        FailIf(
        ...
        FailIf(
            _roomEvents.Active || !impaEvent.Following || _player.CutsceneControlled ||
        ...
        foreach (Vector2I direction in impaDirections)
        {
            ...
            FailIf(
                impa.CurrentAnimationOpaquePixels == 0 ||
        ...
        FailIf(impa.Position != followStart, "Impa advanced before the 16-entry Link path delay elapsed.");
        ...
        FailIf(
            incomingImpa is null || incomingImpa == impa ||
        ...
        FailIf(
            returningImpas.Count != 2 || returningFollower is null ||
            incomingImpa.Active ||
        ...
        FailIf(
            gatheringFollower is null || !impaEvent.Following ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            gatheringFollower.Position != _player.Position + Vector2.Left * 16 ||
        ...
        FailIf(
            returningGatheringFollower is null || !impaEvent.Following ||
        ...
        FailIf(
            completedImpa is null || completedImpa.Active || _roomEvents.Active ||
        ...
        impaEvent.UpdateHelpFrame(upPressed: true);
        FailIf(
            impaEvent.HelpWaitingAtEdge || _roomEvents.Active || _dialogue.IsOpen,
        ...
        FailIf(
            helpStarts.Length != expectedHelpOpcodes.Length ||
        ...
        CutsceneCommandTraceEntry[] helpWaitUpdates = encounterTrace.Entries
            ...
            .ToArray();
        FailIf(
            helpStarts[0].ScriptUpdate != helpStarts[1].ScriptUpdate ||
        ...
        int CompletedUpdate(int commandIndex) => encounterTrace.Entries.Single(entry =>
            ...
            entry.Phase == CutsceneCommandTracePhase.Completed).ScriptUpdate;
        FailIf(
            starts[1].ScriptUpdate != CompletedUpdate(0) + 1 ||
        ...
        ValidationCutsceneTrace commandTrace)
    {
        const int group = 0;
        const int room = 0x59;
        ImpaStoneEventRecord record = impaEvent.Database.StoneRecord;
        ImpaStoneActorRecord stone = record.Actor;
        ImpaStoneTimingRecord timing = record.Timing;
        ...
        FailIf(
            follower is null || stoneActor is null ||
            impaEvent.CurrentStoneStage != StoneStage.WaitingForApproach ||
        ...
        FailIf(
            !_player.CutsceneControlled || !_roomEvents.Active ||
            impaEvent.CurrentStoneStage != StoneStage.SpotJumpHold ||
        ...
        while (impaEvent.CurrentStoneStage == StoneStage.SpotJumpAir &&
            firstAirUpdates++ < 60)
        {
            StepRoomEventFrames(1);
        }
        FailIf(
            firstAirUpdates != 29 ||
            impaEvent.CurrentStoneStage != StoneStage.FirstLandingWait ||
        ...
        FailIf(
            ...
            "Impa did not show TX_0104 after the first landing wait.");
        _dialogue.Close();
        StepRoomEventFrames(1 + timing.FirstTextPostFrames);
        int approachUpdates = 0;
        while (impaEvent.CurrentStoneStage == StoneStage.ApproachStone &&
            approachUpdates++ < 80)
        {
            StepRoomEventFrames(1);
        }
        FailIf(
            impaEvent.CurrentStoneStage != StoneStage.AtStoneWait ||
        ...
        while (impaEvent.CurrentStoneStage == StoneStage.SecondJumpAir &&
            secondAirUpdates++ < 60)
        {
            StepRoomEventFrames(1);
        }
        FailIf(
            secondAirUpdates != 25 ||
            impaEvent.CurrentStoneStage != StoneStage.SecondLandingWait,
        ...
        FailIf(
            ...
            "Impa did not show expanded TX_0105 with the Triforce glyph.");
        _dialogue.Close();
        ...
        while (!_dialogue.IsOpen && firstRetreatUpdates++ < 120)
            StepRoomEventFrames(1);
        FailIf(
            firstRetreatUpdates != 98 || !_dialogue.IsOpen || _dialogue.CurrentMessage !=
        ...
        while (!_dialogue.IsOpen && secondRetreatUpdates++ < 120)
            StepRoomEventFrames(1);
        FailIf(
            secondRetreatUpdates != 95 || !_dialogue.IsOpen || _dialogue.CurrentMessage !=
        ...
        while (impaEvent.CurrentStoneStage != StoneStage.WaitingForPush &&
            finishRetreatUpdates++ < 60)
        {
            StepRoomEventFrames(1);
        }
        FailIf(
            finishRetreatUpdates != 31 ||
            impaEvent.CurrentStoneStage != StoneStage.WaitingForPush ||
        ...
        FailIf(
            prePushStarts.Length != expectedPrePushOpcodes.Length ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            !impaEvent.WaitingNpcInitialized ||
            !_entities.BlocksLink(follower.Position) ||
        ...
        FailIf(
            _player.Position != follower.Position + Vector2.Left * 12,
        ...
        FailIf(
            !_dialogue.IsOpen || _player.Position.Y != stone.LeaveY ||
        ...
        FailIf(
            _player.Position != new Vector2(stone.InitialX - 16, stone.InitialY) ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            impaEvent.StonePushCounter != timing.PushHoldFrames || _player.IsPushing,
            "Releasing the left-side stone push did not reset its hold counter.");

        _player.Face(Vector2I.Right);
        impaEvent.UpdateStoneFrame(pushing: false);
        FailIf(
            impaEvent.StonePushCounter != timing.PushHoldFrames,
            "The stone push counter did not reset to $14.");
        ...
        FailIf(
            ...
            "The Triforce stone moved before 20 centered push updates.");
        StepRoomEventFrames(1);
        Input.ActionRelease("move_right");
        FailIf(
            impaEvent.CurrentStoneStage != StoneStage.PushStarted ||
        ...
        while (!_dialogue.IsOpen && responseUpdates++ < 400)
        {
                ...
                entry.Phase == CutsceneCommandTracePhase.Started))
            {
                ...
                FailIf(
                    follower.CurrentScriptAnimationSource !=
        ...
        while (impaEvent.CurrentStoneStage != StoneStage.Moved &&
            finishUpdates++ < 100)
        {
            StepRoomEventFrames(1);
        ...
        FailIf(
            PrimaryRightCompleted(1).NextCommandIndex != 6 ||
        ...
        ValidationImpaPostPushHost leftPostPushHost = new ValidationImpaPostPushHost(linkAngle: 0x18);
        ...
        CutsceneCommandTraceEntry LeftCompleted(int commandIndex) =>
            leftPostPushHost.Trace.Entries.Single(entry =>
                ...
                entry.Phase == CutsceneCommandTracePhase.Completed);
        FailIf(
            leftPostPushRunner.Active || !leftPostPushHost.Ended ||
    ...
    private void ValidateMakuTreeDisappearanceCutscene()
    {
        ...
        FailIf(
            makuTree is null || !_roomEvents.Active || !makuTree.Active ||
        ...
        FailIf(
            makuRecord.InitialPaletteHeader != 2 ||
        ...
        FailIf(
            makuTree.CurrentAnimationTextureSize.X <= 32 ||
        ...
        FailIf(
            _player.Position != inputStart,
            "Maku Tree simulated input did not begin with 60 idle updates.");
        StepRoomEventFrames(48);
        FailIf(
            makuEvent.InputFrame != 108 || _player.FacingVector != Vector2I.Right ||
        ...
        FailIf(
            makuEvent.PaletteHeader == paletteBefore,
            "The $9a/$c4/$8f/$c5 Maku Tree palettes did not cycle within eight updates.");
        StepRoomEventFrames(202);
        FailIf(
            !_dialogue.IsOpen || _dialogue.Position.Y != 96 || _dialogue.CurrentMessage != "Ahh...",
            "TX_0540 did not open after 210 disappearance updates.");
        ...
        FailIf(
            ...
            "TX_0541 did not follow the original 210-update pause.");
        ...
        FailIf(
            _currentRoom.TilesetId != 0x22 || !makuTree.Active || _warpFade.Color.A != 0.0f ||
        ...
        FailIf(
            reloadedTree is null || reloadedTree.Active || GateMatchesExpected(retiredGate) ||
        ...
        _player._PhysicsProcess(1.0 / 60.0);
        FailIf(
            _activeGroup != 0 || _currentRoom.Id != 0x38 || IsTransitioning,
        ...
        FailIf(
            _roomEvents.Active || reloadedTree is null || reloadedTree.Active,
            "The completed Maku Tree entry event retriggered on room reload.");
        ...
        int CompletedUpdate(int commandIndex) => commandTrace.Entries.Single(entry =>
            ...
            entry.Phase == CutsceneCommandTracePhase.Completed).ScriptUpdate;
        FailIf(
            starts[1].ScriptUpdate != starts[0].ScriptUpdate ||
    ...
    private void ValidateMakuSproutRescueCutscene()
    {
        ...
        FailIf(
            rescue.HasState || _roomEvents.Active,
        ...
        MakuSproutRescueDatabaseActorRecord leftActor = database.Actors["MoblinLeft"];
        MakuSproutRescueDatabaseActorRecord rightActor = database.Actors["MoblinRight"];
        ...
        FailIf(
            initialSprout is null || initialLeft is null || initialRight is null ||
        ...
        FailIf(
            IsTransitioning || _activeGroup != group || _currentRoom.Id != 0x48 ||
        ...
        FailIf(
            requiredScripts.Any(script => !scripts.Contains(script)) ||
        ...
        FailIf(
            rescue.HasState || _roomEvents.Active || savedSprout is null ||
        ...
        MaskedMoblinRecord masked = enemyDatabase.MaskedMoblin;
        ...
        FailIf(
            deflectedArrow.Finished || deflectedArrow.Counter != 1 ||
            deflectedArrow.ZFixed == 0,
            "PART_ENEMY_ARROW ended before its $20 SPEED_40 -$00e0/$0e bounce completed.");
        deflectedArrow.UpdateFrame(_player);
        FailIf(!deflectedArrow.Finished, "PART_ENEMY_ARROW did not delete on bounce counter zero.");
        deflectedArrow.Free();
        ...
        FailIf(
            adviceSprout.CurrentScriptAnimationSource !=
    ...
    private void ValidateMakuTreeSavedCutscene()
    {
        ...
        NpcCharacter tree = _entities.Entities<NpcCharacter>().Single(npc =>
            npc.Record.Id == record.InteractionId && npc.Record.SubId == record.SubId);
        FailIf(
            !savedEvent.HasState || savedEvent.BlocksGameplay || !tree.Active ||
        ...
        StepRoomEventFrames(3);
        FailIf(
            !savedEvent.ButtonSensitive || savedEvent.CurrentCommandIndex != 6 ||
        ...
        for (int frame = 0; frame < 5000; frame++)
        {
            ...
            if (_dialogue.IsOpen && textStarts.Length > handledTextStarts)
            {
                ...
                CutsceneCommand command =
                    database.Commands[started.Source.CommandIndex];
                FailIf(
                    command is not CutsceneShowTextCommand,
                    "Saved Maku Tree showtext trace did not resolve to typed text metadata.");
                var text = (CutsceneShowTextCommand)command;
        ...
        FailIf(
            !textIds.SequenceEqual(expectedTexts) || choiceCount != 2 ||
        ...
        StepRoomEventFrames(2);
        FailIf(
            savedEvent.CurrentCommandIndex != 60 ||
            respawned.State != PickupState.Waiting ||
            _dialogue.IsOpen,
        ...
        _entities.Update(1.0 / 60.0, _player);
        FailIf(
            _interactions.GroundTreasureForValidation != respawned ||
            !_saveData.HasRoomFlag(group, room, OracleSaveData.RoomFlagItem) ||
        ...
        FailIf(reloadInventory is null, "Could not restore validation inventory state.");
    ...
    private void ValidateRoom056Comedian()
    {
        ...
        System.Reflection.MethodInfo? reloadInventory = typeof(InventoryState).GetMethod(
            ...
            System.Reflection.BindingFlags.Instance |
            System.Reflection.BindingFlags.NonPublic);
        FailIf(reloadInventory is null, "Could not reload comedian validation inventory.");
        ...
        void FinishSimpleTalk()
        {
            ...
            StepRoomEventFrames(31);
            FailIf(
                comedianEvent.BlocksGameplay || _player.CutsceneControlled ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            comedian.CurrentScriptAnimationSource != record.Animation0,
        ...
        LoadValidationRoom(group, room);
        FailIf(comedianEvent.Progress != 1, "Essence bit 1 did not select comedian progress 1.");
        BeginTalk();
        ...
        LoadValidationRoom(group, room);
        FailIf(comedianEvent.Progress != 2, "Essence bit 2 did not select comedian progress 2.");
        BeginTalk();
        ...
        StepRoomEventFrames(31);
        FailIf(
            comedianEvent.BlocksGameplay || comedianEvent.CurrentCommandIndex != 8,
    ...
    private void ValidateNayruIntroCutscene()
    {
        ...
        FailIf(
            actors.Count != expectedActors.Length || nayruIntro.CurrentStage != 1 ||
            _roomEvents.Active || _player.CutsceneControlled ||
        ...
        FailIf(
            ...
            nayruDatabase.Vignette(0) != new VignetteRecord(0, 0, 0x98, 937) ||
            nayruDatabase.Vignette(1) != new VignetteRecord(1, 0, 0x5a, 600) ||
            nayruDatabase.Vignette(2) != new VignetteRecord(2, 2, 0x0e, 645) ||
            nayruDatabase.VignetteMonkeys.Count != 10 ||
            nayruDatabase.VignetteMonkeys[8] !=
                new VignetteMonkeyRecord(8, 0x50, 0x46, 180, 2) ||
        ...
        foreach ((string name, Vector2 position) in expectedActors)
        {
            ...
            FailIf(
                !actors.TryGetValue(name, out NpcCharacter? actor) || !actor.Active ||
        ...
        FailIf(
            actors["Nayru"].SourceGraphicsWidth != 256,
        ...
        FailIf(
            nayruDatabase.Effect("MusicNote").SpriteName != "spr_common_sprites" ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            bird.ScriptDrawOffset != Vector2.Zero ||
            bird.CurrentScriptAnimationSource != nayruDatabase.Actor("Bird").Animation(2),
            "The intro bird did not stop hopping and hold its talk pose for the post-text wait.");
        StepRoomEventFrames(10);
        FailIf(
            bird.CurrentScriptAnimationSource != nayruDatabase.Actor("Bird").Animation(1),
        ...
        FailIf(
            !nayruIntro.TryInteractNpc(rabbit) ||
        ...
        FailIf(
            !nayruIntro.TryInteractNpc(monkey) ||
        ...
        StepRoomEventFrames(21);
        FailIf(
            nayruTrace.LastValue("AudienceMask") != 0x0f ||
        ...
        FailIf(
            actors["Bear"].Position != new Vector2(0x58, 0x30) ||
        ...
        FailIf(
            ...
            "The bear lead-in did not restore Link control.");

        _sound.ClearPlayRequestAudit();
        _player.WarpTo(new Vector2(0x60, 0x3d), recordSafe: false);
        StepRoomEventFrames(1);
        FailIf(
            nayruIntro.CurrentStage != 6 || nayruIntro.Counter != 120 ||
            !_player.CutsceneControlled,
            "The x>=$60/y<$3e initial Nayru cutscene boundary did not install the 120-update wait.");
        StepRoomEventFrames(120);
        FailIf(
            !_dialogue.IsOpen || _dialogue.CurrentMessage != "Isn't it \nenchanting?",
            "TX_5706 did not follow the bear's 120-update wait.");
        _dialogue.Close();
        StepRoomEventFrames(1);
        StepRoomEventFrames(30);
        FailIf(
            nayruIntro.CurrentStage != 9 || nayruIntro.Counter != 11,
            "The post-TX_5706 30-update delay did not begin the fast fade.");
        StepRoomEventFrames(11);
            ...
            scriptFrames < 20000)
        {
            ...
            if (newVignettes != 0)
            {
                ...
                FailIf(
                    _rooms.ActiveGroup != expected.Group ||
                ...
                aftermathRalph.CurrentScriptAnimationSource == ralphFallAnimation)
            {
                ...
                if (ralphFallFrames > 60)
                {
                    ...
                    FailIf(
                        aftermathRalph.CurrentAnimationFrame != 9,
        ...
        FailIf(portal is null || !portal.Active, "Lightning tile $22=$d7 did not activate portal $e1:$01.");
        ...
        FailIf(
            _currentRoom.GetMetatile(portalPoint) != 0xd7 || portal is null || !portal.Active ||
    ...
    private void ValidateRalphPortalDepartureEvent()
    {
        ...
        FailIf(
            directRalph is null || directRalph.Active || _roomEvents.Active,
        ...
        StepRoomEventFrames(29);
        FailIf(
            ralphEvent.Counter != 1 || ralph.Position != new Vector2(0x18, 0x28),
            "Ralph's post-text 30-update wait ended early.");
        StepRoomEventFrames(1);
        FailIf(
        ...
        StepRoomEventFrames(1);
        FailIf(
            ralphEvent.Counter != 17 || ralph.Position != new Vector2(0x18, 0x28),
            "Ralph did not install applyspeed counter $11 on its own script update.");

        StepRoomEventFrames(12);
        FailIf(
            ralph.Position != new Vector2(0x24, 0x28) ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            ralph.Position != new Vector2(0x25, 0x28) ||
            ralph.CurrentAnimationFrame != 1 || ralphEvent.Counter != 4,
            "Ralph's animation $01 did not change after its original 16 updates.");
        StepRoomEventFrames(2);
        FailIf(
            ralph.Position != new Vector2(0x27, 0x28) || ralphEvent.Counter != 2,
            "Ralph's SPEED_100 movement skipped an update.");
        StepRoomEventFrames(1);
        FailIf(
        ...
        StepRoomEventFrames(1);
        FailIf(
            ralphEvent.Counter != 0 || ralphEvent.Flickering ||
            ralph.Position != new Vector2(0x28, 0x28),
            "Ralph's counter2 path did not pause for one update after reaching zero.");
        StepRoomEventFrames(1);
        FailIf(
            ralph.CurrentAnimationFrame != 0 || ralphEvent.Flickering,
            "Ralph did not select portal animation $09.");
        StepRoomEventFrames(2);
        FailIf(
        ...
        FailIf(
            !ralphEvent.Flickering || ralphEvent.Counter != 44 ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            ralph.Visible == firstFlickerVisibility || ralphEvent.Counter != 43,
            "Ralph's objectFlickerVisibility b=$01 did not alternate every update.");
        StepRoomEventFrames(42);
        FailIf(
            !_roomEvents.Active || ralphEvent.Counter != 1 ||
        ...
        NpcCharacter? completedRalph = _npcNodes.Find(npc =>
            npc.Record.Id == 0x37 && npc.Record.SubId == 0x0d);
        FailIf(
            completedRalph is null || completedRalph.Active || _roomEvents.Active,
            "Ralph's one-shot portal event retriggered after flag $40.");
        ...
        int flickerCompletionUpdate = commandTrace.Entries.Single(entry =>
            ...
            entry.Phase == CutsceneCommandTracePhase.Completed).ScriptUpdate;
        FailIf(
            starts[12].ScriptUpdate != flickerCompletionUpdate ||
