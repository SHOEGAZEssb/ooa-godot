...
public sealed partial class ValidationRoot
{
    private void ValidateRoom07cPoe()
    {
        ...
        originalFrameCounter = (int)frameCounterField!.GetValue(_entities)!;
        originalFrameAccumulator =
            (double)frameAccumulatorField!.GetValue(_entities)!;
        originalRng1 = (byte)rng1Field!.GetValue(_random)!;
        originalRng2 = (byte)rng2Field!.GetValue(_random)!;
        originalRandomCalls = (int)randomCallsField!.GetValue(_random)!;
        originalLastRandom =
            (OracleRandomResult)lastRandomField!.GetValue(_random)!;
        ...
        void BeginTalk(int textId, int expectedVariant)
        {
            ...
            FailIf(
                actor.Record.Var03 != expectedVariant ||
        ...
        FailIf(
            _entities.FindTalkTarget(_player) != first,
        ...
        FailIf(
            !poeEvent.HasState || !first.Active || poeEvent.Counter != 1 ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            poeEvent.HasState || first.Active || first.Visible ||
        ...
        LoadValidationRoom(group, room);
        FailIf(
            Actors().Any(actor => actor.Active) || poeEvent.HasState,
        ...
        FailIf(
            final.Record.Var03 != record.FinalVariant ||
            !poeEvent.ButtonSensitive || poeEvent.CurrentCommandIndex != 1,
    ...
    private void ValidateRoom22ePoe()
    {
        ...
        FailIf(
            _entities.FindTalkTarget(_player) != tombPoe ||
            !_interactions.TryInteract(_player),
        ...
        StepRoomEventFrames(1);
        FailIf(
            poeEvent.CurrentCommandIndex != 17 ||
        ...
        StepRoomEventFrames(1);
        FailIf(
            poeEvent.HasState || tombPoe.Active || tombPoe.Visible ||
            poeEvent.BlocksGameplay || _player.CutsceneControlled,
            "The tomb Poe did not restore input and delete at var3e=$00.");

        LoadValidationRoom(group, room);
        FailIf(
            Actors().Single().Active || poeEvent.HasState,
