...
public sealed partial class ValidationRoot
{
    private void ValidateDarkRoomInteractions()
    {
        ...
        FailIf(
            database.RecordCount != 3 || records.Count != 2 ||
            records[0] is not
            {
        ...
        FailIf(
            _entities.Entities<DarkRoomRewardRoomEntity>().Count != 1 ||
        ...
        FailIf(
            !handler.Initialized || handler.State.TotalTorches != 2 ||
        ...
        for (int update = 0; update < 6; update++)
            _entities.Update(1.0 / 60.0, _player);
        FailIf(
            !handler.State.FadeActive || handler.State.RenderedOffset != -10 ||
        ...
        FailIf(
            _entities.Entities<DarkRoomRewardRoomEntity>().Count != 0 ||
            key.Record is not
            {
                TreasureObject: "TREASURE_OBJECT_GRAVEYARD_KEY_00",
