...
public sealed partial class ValidationRoot
{
    ...
    private void ValidateHarp()
    {
        ...
        if (dormant.Active || dormant.Visible || dormant.Temporary)
        {
            throw new InvalidOperationException(
        ...
        if (_player.IsUsingHarp || _player.HarpPoseActive ||
        ...
        if (!_interactions.DialogueOpen || IsTransitioning)
        {
            throw new InvalidOperationException(
        ...
        if ((_currentRoom.TilesetFlags & 0x80) == 0 ||
        ...
        if (_activeGroup != 0 || _currentRoom.Id != 0x06 ||
        ...
        if (currentsReturn.CurrentPalette == initialPalette)
        {
            throw new InvalidOperationException(
        ...
        {
            throw new InvalidOperationException(
                "Tune of Ages did not time-warp from the present.");
        }
        FinishTimeWarp();
        if (_activeGroup != 1 ||
    ...
    private void ValidateHarpInventorySubmenu()
    {
        ...
        if (!inventory.HasTreasure(TreasureDatabase.TreasureHarp) ||
        ...
        for (int update = 1; update < 16; update++)
            menu.UpdateItemSubmenuForValidation(HarpFrame);
        if (screen.ItemSubmenuReady)
        {
            throw new InvalidOperationException(
                "Three-song Harp submenu opened before source update 17.");
        }
        menu.UpdateItemSubmenuForValidation(HarpFrame);
        if (!screen.ItemSubmenuReady ||
        ...
        if (!menu.MoveItemSubmenuForValidation(-1) ||
        ...
        var twoSongMenu = new InventoryMenuController(
            ...
            () => { },
            _sound.PlaySound);
        if (twoSongMenu.EquipToBForValidation())
            throw new InvalidOperationException(
        ...
        if (twoSongScreen.ItemSubmenuReady ||
        ...
        if (!twoSongScreen.ItemSubmenuReady)
            throw new InvalidOperationException(
                "Two-song Harp submenu was not ready on source update 15.");
    ...
    private void PlaySelectedHarpSong(bool validateNoteSides = false)
    {
        ...
        for (int update = 0; update < expectedFrames; update++)
        {
            ...
            if (validateNoteSides && (update + 1) % 32 == 0)
            {
                ...
                if (!Mathf.IsEqualApprox(note.Position.X, expectedX))
                {
                    throw new InvalidOperationException(
            ...
            if (update + 1 < expectedFrames && !_player.IsUsingHarp)
            {
                throw new InvalidOperationException(
    ...
    private void FinishTimeWarp()
    {
        ...
        {
            throw new InvalidOperationException(
                "Harp time warp did not finish within its source timing bound.");
        }
    }
