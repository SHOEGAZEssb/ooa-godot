...
public sealed partial class ValidationRoot
{
    private void ValidateDarkRoomInteractions()
    {
        ...
        IReadOnlyList<DarkRoomDatabase.Record> records =
            database.GetRoomRecords(group, roomId);
        if (database.RecordCount != 3 || records.Count != 2 ||
            records[0] is not
            {
                Order: 0, Kind: DarkRoomDatabase.ObjectKind.Reward,
        ...
        _entities.Update(1.0 / 60.0, _player);
        _entities.Update(1.0 / 60.0, _player);
        if (key.State != GroundTreasurePickup.PickupState.Spawning ||
            key.SpawnCounter != 40 ||
        ...
        for (int update = 0;
             update < 240 && key.State != GroundTreasurePickup.PickupState.Waiting;
             update++)
        {
            _entities.Update(1.0 / 60.0, _player);
        }
        if (key.State != GroundTreasurePickup.PickupState.Waiting ||
        ...
        if (!_inventory.HasTreasure(TreasureDatabase.TreasureGraveyardKey) ||
            ...
            key.State != GroundTreasurePickup.PickupState.Collected ||
