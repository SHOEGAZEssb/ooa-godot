...
public sealed partial class ValidationRoot
{
    private void ValidateGraveyardCrowsAndDropProducers()
    {
        ...
        NpcDatabase.NpcRecord ghini = npcs.GetRoomNpcs(0, 0x5d).Single(npc =>
        ...
        if (crow.Position != new Vector2(0x78, 0x78) ||
            crow.State != CrowCharacter.CrowState.Perched || crow.CollisionEnabled ||
        ...
        if (!bombProducer.Initialized || crow.State != CrowCharacter.CrowState.Perched)
            throw new InvalidOperationException(
        ...
        manager.Update(1.0 / 60.0, _player);
        if (crow.State != CrowCharacter.CrowState.Rising || crow.Counter1 != 25)
            throw new InvalidOperationException(
        ...
        if (aimDelta > 0x10)
            aimDelta -= 0x20;
        if (crow.State != CrowCharacter.CrowState.Charging || crow.Z != -6 ||
        ...
        if (manager.Entities<ItemDropProducer>().Count != 0 ||
            ...
            bombDrop.State != ItemDropEffect.DropState.Bouncing ||
