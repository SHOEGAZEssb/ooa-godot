...
public sealed partial class ValidationRoot
{
    private void ValidateOverworldKeyholeAndGraveyardGate()
    {
        ...
        if (database.Count != 6 || database.TileCount != 3 ||
            !database.TryGet(group, roomId, out OverworldKeyholeDatabase.Record keyhole) ||
            keyhole is not
            {
        ...
        if (_inventory.HasTreasure(TreasureDatabase.TreasureGraveyardKey) ||
            ...
            gate.Stage != GraveyardGateEvent.EventStage.WaitingForKeyhole ||
        ...
        for (int frame = 0; frame < 9; frame++)
            Push();
        if (gate.Stage != GraveyardGateEvent.EventStage.WaitingForKeyhole ||
        ...
        if (!_inventory.HasTreasure(TreasureDatabase.TreasureGraveyardKey) ||
            !_saveData.HasRoomFlag(group, roomId, OracleSaveData.RoomFlag80) ||
            gate.Stage != GraveyardGateEvent.EventStage.Running ||
        ...
        StepGate();
        if (gate.Stage != GraveyardGateEvent.EventStage.Completed ||
        ...
        if (room.GetMetatile(new Vector2(0x48, 0x38)) != 0x3a ||
            ...
            gate.Stage != GraveyardGateEvent.EventStage.Inactive)
        {
            throw new InvalidOperationException(
